...
if(isset($_GET['Kode'])) {
	...
	$cekSql = "SELECT * FROM tmp_keranjang WHERE kd_homestay='$Kode' AND kd_pelanggan='$KodePelanggan'";
	$cekQry = mysql_query($cekSql, $koneksidb) or die ("Cek data homestay".mysql_error());
	if(mysql_num_rows($cekQry) >=1) {
		// Jika barang sudah pernah dipilih, maka update saja jumlah barangnya (+1)
		$mySql = "UPDATE tmp_keranjang SET jumlah=jumlah + 1 WHERE kd_homestay='$Kode' AND kd_pelanggan='$KodePelanggan'";

	}
	else {
		// Jika barang belum pernah dipilih, maka tambahkan baris baru ke keranjang
		$mySql = "SELECT * FROM homestay WHERE kd_homestay='$Kode'";
		$myQry = mysql_query($mySql, $koneksidb) or die ("Gagal ambil data homestay".mysql_error());
		$myData = mysql_fetch_array($myQry);
		
		// Membaca data dari tabel Homestay, untuk diinput ke tabel TMP
		$hargaModal	= $myData['harga_promo'];
		$hargaJual	= $myData['harga_jual'];
		$tanggal	= date('Y-m-d');
		
		// Simpan data ke TMP (Keranjang Belanja)
		$mySql	= "INSERT INTO tmp_keranjang (kd_homestay, harga, jumlah, tanggal, kd_pelanggan) 
					VALUES('$Kode', '$hargaJual', '1', '$tanggal', '$KodePelanggan')";
	}
	
	// Menjalankan SQL di atas ( Update jumlah homestay & Input homestay baru ke TMP)
